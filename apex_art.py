...
def type_css(look, sel_xl=".hxl", sel_lg=".hlg"):
    ...
    css = f"{sel_xl}{{font-weight:{w};letter-spacing:{tr_xl}px}} {sel_lg}{{font-weight:{w};letter-spacing:{tr_lg}px}}"
    if abs(sc-1.0) > 0.001:
...
def build_style_block(look):
    ...
    kin = {"mode": em,
           "unit_stagger": 0.06 if em != "word_stagger" else 0.05,
...
def remember_look(look):
    try:
        nm = look.get("lookbook"); rec = load_recent()
        if rec and rec[-1] == nm: return
        rec.append(nm); rec = rec[-6:]
        with open(MEMORY, "w", encoding="utf-8") as f: json.dump({"recent": rec}, f)
    except Exception:
        pass
